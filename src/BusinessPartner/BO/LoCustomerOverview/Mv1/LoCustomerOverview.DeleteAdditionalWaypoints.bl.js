...
overviewList.forEach(function (visit) {
  if (
    visit.getClbStatus() == BLConstants.VISIT.STATUS_PLANNED ||
    visit.getClbStatus() == BLConstants.VISIT.STATUS_INPROGRESS
  );
  var visitPin = me.getItemByPKey(visit.bpaMainPKey);
  if (Utils.isDefined(visitPin)) {
    visitPin.setVisibleInMap("0"); //Remove visibility of Purple Pins if there is a corresponding call Planned already.
  }
});
