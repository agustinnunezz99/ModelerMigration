 ...
 * @param {Object} boCall
 * @returns posId
 * @description Function is loading survey products of the given POS and sets the promotion and discrepancy filters.
 *  If the visit is in readOnly mode survey columns are not editable.
 */
...
if (loCurrentPOS.length > 0) {
  ...
  if (!Utils.isDefined(loProducts)) {
    loProducts = BoFactory.instantiate("LoJobProducts");
    ...
    //survey columns must not be editable if visit is read onyl
