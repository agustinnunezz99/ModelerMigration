...
function loadInventoryDocumentItems(){
    ...
    if (me.getDocumentType() === BLConstants.Order.DOCUMENT_TYPE_PRODUCT_CHECK_IN) {
        ...
        //########################################
        var fetchCheckInItemsPromise = me.getPhase() != BLConstants.Order.PHASE_RELEASED ? me.fetchCheckInItems() : when.resolve([]);
        ...
        promise = fetchCheckInItemsPromise.then(
            function (resultFetchCheckInItems) {
                ...
                return Facade.executeQueries([statementPersistedCheckInItems]).then(
                    function (persistedCheckInItems) {
                        ...
                        return BoFactory.createListAsync(LO_INVENTORYDOCUMENTITEMS, {}).then(
                            function (loItemsMainList) {
                                ...
                                me.populateCheckInItems(inventoryCheckInItems, persistedCheckInItems[0]);
                            });
                    }
                );
            }
