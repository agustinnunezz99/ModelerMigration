...
if(!Utils.isEmptyString(me.getCallInContext_clbMainPKey())) {
  promise = Facade.getObjectAsync("BoCall", me.getQueryBy("pKey", me.getCallInContext_clbMainPKey()))
    ...
    var isDocumentTypeDelivery =
      documentType === BLConstants.Order.DOCUMENT_TYPE_DELIVERY;

    if (Utils.isDefined(boCall) && (isDSDMode || isDocumentTypeDelivery)) {
      var visitStatus = boCall.clbStatus;
      var isVisitStatusNonEditable =
        visitStatus === BLConstants.VISIT.STATUS_COMPLETED ||
        visitStatus === BLConstants.VISIT.STATUS_CANCELLED ||
        (isDSDMode && visitStatus === BLConstants.VISIT.STATUS_PLANNED);
      // Delivery documents should be read-only, regardless of the visit status.
      var bCallIsReadOnly = isDocumentTypeDelivery || isVisitStatusNonEditable;
